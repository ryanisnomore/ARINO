...
module.exports = class Music {
  ...
  async handleError(ctx, err) {
    if (ctx.type === InteractionType.ApplicationCommand) {
      await ctx.editReply({
        content:
          "- An error occurred while playing the song. Try Reporting it to the Developer [Team Arino](<https://discord.gg/W2GheK3F9m>)"
      });
  ...
  async Skip(ctx) {
    try {
      ...
      player.skip();
      const newMsg = `<:HvnTick:1270904627088592918> **${this.Name(
        ctx
  ...
  async Disconnect(ctx) {
    try {
      ...
      // }
      const newMsg = `<:HvnTick:1270904627088592918> **Disconnected** From Voice Channel!`;
      await this.ctxSend(ctx, { content: newMsg });
    } catch (err) {
      this.console.error(err);
    }
  ...
  \`\`\``;
      ...
      const QueuePage = `## <:author:1272049050438602752> Queue Of ${
        ctx.guild.name
      }\n${
      ...
      collector.on("collect", async (i) => {
        ...
        if (i.customId === "previous_q") {
          ...
          const QueuePage = `## <:author:1272049050438602752> Queue Of ${ctx.guild.name}\n${pageMsg}\n\n**Page ${page}/${totalPages}**`;
