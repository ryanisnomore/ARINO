...
module.exports = async (client, message) => {
  try {
    ...
    if (message.content.match(mentionRegex)) {
      const msg =
        Math.random() > 0.5
          ? `> <:aHumanity_Friends:1265806769331109912> Greetings <@!${message.author.id}>! I am ${client.user.username}, a user-friendly bot with unique features.\n- Join [Support Server](https://discord.gg/W2GheK3F9m)\n- Invite Me [${client.user.username}](https://discord.com/oauth2/authorize?client_id=${client.user.id}&permissions=8&scope=bot%20applications.commands)\n- My Prefix is \`${prefix}\``
          : `> <:aHumanity_Friends:1265806769331109912> Hola **${message.author.globalName}**! This is ${client.user.username}, a legendary music bot.\n- Join [Support Server](https://discord.gg/W2GheK3F9m)\n- Invite Me [${client.user.username}](https://discord.com/oauth2/authorize?client_id=${client.user.id}&permissions=8&scope=bot%20applications.commands)\n- My Prefix is \`${prefix}\``;

      const embed = new EmbedBuilder()
        .setAuthor({
          name: `Arino Music For You`,
          iconURL: client.user.displayAvatarURL({ dynamic: true }),
        })
        .setColor(client.config.color)
        .setDescription(`${msg}\n-# Special Thanks To All Supporters ❤`)
        .setImage(
          "https://media.discordapp.net/attachments/1266081851714441267/1272359687089553460/1720359035935-1.png"
        );
      ...
      const actionRow = new ActionRowBuilder().addComponents(
        new ButtonBuilder()
          ...
          .setURL("https://discord.gg/W2GheK3F9m"),
        new ButtonBuilder()
          .setLabel("Invite Me")
          .setStyle(ButtonStyle.Link)
          .setURL(
    ...
    const checkBotPermissions = async (message, client) => {
      ...
      try {
        ...
        if (mPermissions.length > 0) {
          ...
          const content = `- I need these many permissions ${permissionList}. Consider Reporting To Developer [Arshman Naim Ryan (RY4N)](<https://discord.com/users/1085376019445321829>)`;
          try {
    ...
    if (cmd.permission && !message.member.permissions.has(cmd.permission) && !dev.includes(message.author.id)) {
      return message.channel
        ...
        .catch(() => {
          message.author
            .send({
              content: `- <:supp:1333097493298286624> I cannot send messages in ${message.channel.name}. Please ensure I have permissions to send messages in that channel! [Re Invite Me](https://discord.com/oauth2/authorize?client_id=${client.user.id}&permissions=8&scope=bot%20applications.commands)`,
            })
            .catch(() => {});
        });
    ...
    if (cmd.dev && !client.config.developers.includes(message.author.id)) {
      return message.channel.send({
        content: `- This command is restricted to Arshman Naim Ryan (RY4N)`,
      });
    }
    if (
      cmd.options.owner &&
      ...
      return message.channel.send({
        content: `- This command is restricted to KARTIK, DENJI, RiN, RY4N`,
      });
    }
        ...
        return await message.channel.send({
          embeds: [
            new EmbedBuilder()
              ...
              .setDescription(
                `<:topgg:1333099085485572239> <@!${message.author.id}> You need to vote Arino
In Top.gg to use this command. [Vote Here](${client.config.vote})`
              ),
          ],
          components: [
    ...
    await cmd
      ...
      .catch((error) => {
        ...
        message.channel
          .send({
            content: `I encountered an error while executing the command. Please Report To Developer [Arshman Naim Ryan (RY4N)](<https://discord.com/users/1085376019445321829>)`,
          })
          .catch(() => {
            message.author
              .send({
