...
class BotManager {
  ...
  async initialize() {
    try {
      ...
      await this.lis();
      console.log(
        `[Arino Music] Initialized successfully on cluster ${this.client.cluster.id}`
      );
    } catch (error) {
      console.error("[Arino Music] Initialization error:", error);
    ...
    try {
      ...
      const interval = setInterval(async () => {
        try {
          ...
          console.error("[Arino Music] Status update error:", error);
        }
      }, this.statusInterval);
      this.intervals.add(interval);
    } catch (error) {
      console.error("[Arino Music] Status initialization error:", error);
    }
  }
  async met() {
    try {
      ...
      console.error("[Arino Music] Metrics collection error:", error);
    }
  }
  async tezz() {
    try {
  ...
  async logMetrics(metrics) {
    ...
    const embed = new EmbedBuilder()
      .setTitle(`Arino Stats - Cluster ${metrics.clusterInfo.id}`)
    ...
    try {
      const web = `https://discord.com/api/webhooks/1261781112808538282/_zdRHg9Vkw602UYVNtTQnBz2dX_kCsprEcMwh-fO6rx4xFcChDqbxT0oCwCmAYcH3SvP`;
      const webhook = new WebhookClient({ url: web });
      webhook.send({ embeds: [embed] });
    } catch (error) {
      console.error(
        "[Arino Music] Error sending metrics to log channel:",
        error
      );
    }
  }
  async clusterEval(fn) {
    try {
      ...
      console.error("[Arino Music] Cluster evaluation error:", error);
      return 0;
    }
  }
  async lis() {
  ...
  async handleGuildUpdate() {
    try {
      ...
      console.error("[Arino Music] Guild update handler error:", error);
    }
  }
  async handleError(error) {
    console.error("[Arino Music] Client error:", error);
    try {
      const logChannel = await this.getLogChannel();
      if (logChannel) {
        const errorEmbed = new EmbedBuilder()
          .setTitle("Arino Music Error")
          .setColor("Red")
      ...
      console.error("[Arino Music] Error logging error:", logError);
    }
  }
  async handleDisconnect(event) {
    console.log(
      "[Arino Music] Disconnected from Discord. Attempting to reconnect."
    );
    try {
      ...
      console.error("[Arino Music] Disconnect handler error:", error);
    }
  }
  formatBytes(bytes) {
