...
const { avonRecommendations } = require("../Handlers/ArinoRec");
...
module.exports = class AvonClient extends Client {
  constructor() {
    ...
    this.rest = new REST({ version: "10" }).setToken(this.config.token.ARINO);
    this.btn = {
    ...
    try {
      ...
      for (const guild of twentyFourSevenGuilds) {
        try {
          ...
          console.log(
            `Connected to 24/7 VC: ${guild.id}`
          );
        } catch (error) {
          console.error(
            `Failed to connect to 24/7 VC: ${guild.id}`,
            error
          );
        }
      }
  ...
  async connectToDatabase() {
    ...
    await mongoose
      .connect(this.config.mongoURL.RY4N, Options)
      .then(() => console.log("RY4N MongoDB Connected"))
      .catch(async (error) => {
        console.error("Error connecting to MongoDB:", error);
        await mongoose
          .connect(this.config.mongoURL.Secondary, Options)
          .then(() => console.log("Connected to Secondary MongoDB"))
          .catch((error) => console.error("Error connecting to Secondary MongoDB:", error));
      });
  }
  ...
  async AvonBuild() {
    try {
      ...
      await this.login(this.config.token.ARINO).catch(async (error) => {
        console.error(
